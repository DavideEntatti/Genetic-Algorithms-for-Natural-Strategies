...
def run_benchmark(settings_path):
    with open(settings_path, 'r') as f:
        ...
        enabled_algorithms = [
            algorithm
            for algorithm, enabled in (("vitamin", vitamin), ("ga", ga))
            if enabled
        ]
        ...
        with output_path.open("w", newline="") as out:
            ...
            while(current_config < num_configs):
                ...
                while(current_iter < iterations):
                    ...
                    runs = run_case(model_path, formula_path, vitamin=vitamin, ga=ga, timeout=timeout)
                    if vitamin:
                ...
                for algorithm, runs_for_algorithm in algorithm_runs.items():
                    is_win = lambda run: (
                        run[0] is True if algorithm == "vitamin" else run[0] == "Found"
                    )
