...
def search_strategy(formula, model_path):
        ...
        n = len(coalition) * len(agents_actions[0]) * k * len(aps)
        n = int(sqrt(n))
        n_spec = int(n/10)
        config.set_config(pop_size=n, gens=n, elites=n_spec, extras=n_spec, tournament=min(n_spec, 1))
        ...
        while (k_iter < config.DYNAMIC_K or k_iter == 0):
            ...
            while i < config.MAX_GENERATIONS:
                ...
                row = {"Gen":i,"Fit":average_fitness,"Sol":found_new_solution,"F":formula}
